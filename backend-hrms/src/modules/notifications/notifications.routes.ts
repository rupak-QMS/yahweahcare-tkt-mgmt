...
// POST /notifications/send-report-email — send an ad-hoc report email via Resend
router.post('/send-report-email', async (req, res, next) => {
  try {
    ...
    // Build HTML table of report data
    ...
    const dataRows: unknown[][] = Array.isArray(rows) ? rows.slice(0, 200) : [];
    ...
    const tableHtml = hdrs.length ? `
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin:16px 0;font-size:12px;">
        ...
        <tbody>
          ...
          ${dataRows.length === 0 ? `<tr><td colspan="${hdrs.length}" style="padding:16px;text-align:center;color:#6B7280;">No records match the selected filters</td></tr>` : ''}
        </tbody>
      </table>` : '';
  ...
  <tr><td align="center">
    <table width="660" cellpadding="0" cellspacing="0" style="max-width:660px;width:100%;">
      ...
      <tr><td style="background:#fff;padding:28px 32px;border-left:1px solid #E5E7EB;border-right:1px solid #E5E7EB;">
        ...
        <p style="margin:0 0 20px;font-size:13px;color:#6B7280;">Period: <strong>${safePeriod}</strong> &nbsp;·&nbsp; Generated by: <strong>${safeBy}</strong> &nbsp;·&nbsp; ${new Date().toLocaleString('en-AU')}</p>
        <p style="margin:0 0 8px;font-size:13px;color:#374151;font-weight:600;">${dataRows.length} record${dataRows.length!==1?'s':''} in this report${dataRows.length===200?' (first 200 shown)':''}:</p>
        ${tableHtml}
      </td></tr>
    ...
    await sendEmail(recipients, safeSubject, html);
    res.json({ ok: true, recipients, count: dataRows.length });
  } catch (err) { next(err); }
});
